document.addEventListener('DOMContentLoaded', function () {
    ...
    function runMonteCarlo() {
        const sims = 1000;
        const confidence = parseInt(document.getElementById('confidenceSlider').value) || 80;
        const sigma = (100 - confidence) / 100 / Math.sqrt(52);
        const sigmaSq_half = 0.5 * sigma * sigma; // Drift correction
        ...
        for (let s = 0; s < sims; s++) {
            ...
            for (let w = 0; w < 53; w++) {
                ...
                m *= Math.exp(sigma * Z - sigmaSq_half);
    ...
    let sC = null, rC = null;
    function renderDSPTable(preserveFocus = false) {
        ...
        if (preserveFocus && document.activeElement) {
            ...
            if (active.classList.contains('dsp-input') || active.classList.contains('dsp-name-input')) {
                focusedIndex = active.dataset.index;
                focusedClass = active.classList.contains('share-input') ? 'share-input' :
                    active.classList.contains('value-input') ? 'value-input' : 'dsp-name-input';
        ...
        dspData.forEach((d, i) => {
            ...
            b.innerHTML += `<tr><td>${d.isOther ? d.name : `<input type="text" class="dsp-name-input" value="${d.name}" data-index="${i}">`}</td>
                ...
                <td>${Math.round(s).toLocaleString()}</td><td>$${Math.round(r).toLocaleString()}</td></tr>`;
        });

        // Restore focus after re-rendering
        if (preserveFocus && focusedIndex !== null && focusedClass !== null) {
            setTimeout(() => {
                const inputs = document.querySelectorAll(`[data-index="${focusedIndex}"].${focusedClass}`);
                if (inputs.length > 0) {
        ...
        document.getElementById('totalRevenue').textContent = `$${Math.round(totR).toLocaleString()}`;
        const cfg = (d, t) => ({ type: 'pie', data: { labels: dspData.map(x => x.name), datasets: [{ data: d, backgroundColor: ['#1DB954', '#FA243C', '#FF9900', '#FF0000', '#9CA3AF'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: '#fff', usePointStyle: true, pointStyle: 'rectRounded', padding: 15, boxWidth: 12 } }, title: { display: true, text: t, color: '#fff' } } } });
    ...
    document.addEventListener('change', (e) => {
        ...
        if (e.target.classList.contains('share-input')) {
            ...
            renderDSPTable(true); // Preserve focus
        } else if (e.target.classList.contains('value-input')) {
            dspData[i].value = parseFloat(e.target.value) || 0;
            renderDSPTable(true); // Preserve focus
        } else if (e.target.classList.contains('dsp-name-input')) {
            dspData[i].name = e.target.value;
            renderDSPTable(true); // Preserve focus
        }
    });

    document.addEventListener('input', (e) => {
        if (e.target.id === 'confidenceSlider') {
    ...
    function undo() {
        ...
        chart.data.datasets[0].data = undoStack.pop();
        chart.update();
        updateBtns();
    }
    ...
    function redo() {
        ...
        undoStack.push([...chart.data.datasets[0].data]);
        chart.data.datasets[0].data = redoStack.pop();
        chart.update();
        updateBtns();
    }
